...
export const DeviceCard = ({ item, itemType, isShowDiscount = false, }) => {
    ...
    console.log({ item });
    return (<Link to={`/${itemType}/${item.id}`} className="no-underline">
      <div className="bg-white border-solid border-colorLightGrey p-4 flex flex-col max-w-[272px] hover:shadow-lg transition-shadow duration-200">
        <div>
          <div className="flex justify-center mb-4">
            <img src={`/${imageToShow}`} alt={item.name} className="w-[150px] h-[200px] object-contain transform transition-transform duration-300 hover:scale-110"/>
          </div>
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            {isShowDiscount && item.priceDiscount && (<p className="text-gray-500 line-through">${item.priceRegular}</p>)}
          </div>
        ...
        <div className="mt-0 flex gap-2">
          <button className={`w-full px-4 py-2 border ${isInCart
            ? 'bg-white text-green-500 border-colorLightGrey border-solid'
            : 'bg-black text-white'}`} onClick={(e) => {
            e.preventDefault();
            handleCartClick();
        }}>
            {isInCart ? 'Added to cart' : 'Add to cart'}
          </button>
          <button className="w-10 h-10 flex justify-center items-center border-solid border-colorLightGrey" onClick={(e) => {
            e.preventDefault();
