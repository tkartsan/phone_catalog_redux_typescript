...
export const DeviceSlider = ({ items, title, itemType, isShowDiscount = false, sliderId = '', }) => {
    const prevButtonClass = `swiper-prev-${sliderId}`;
    const nextButtonClass = `swiper-next-${sliderId}`;
    return (<div className="mx-auto max-w-[1136px]">
      ...
      <Swiper spaceBetween={15} slidesPerView={4} modules={[Navigation]} navigation={{
            nextEl: `.${nextButtonClass}`,
            prevEl: `.${prevButtonClass}`,
        }}>
        {items.map((item) => (<SwiperSlide key={item.id}>
            <DeviceCard item={item} itemType={itemType} isShowDiscount={isShowDiscount}/>
          </SwiperSlide>))}
      </Swiper>
    </div>);
};
